...
export function getEmptyPos(grid) {
  const ret = {};
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      if (grid[i][j] === 0) {
...
export function getGridObj() {
  ...
  const { empty_i, empty_j } = getEmptyPos(grid);
...
export function invCount(arr) {
  ...
  for (let i = 0; i < arr.length - 1; ++i) {
    for (let j = i + 1; j < arr.length; ++j) {
      if (arr[i] > arr[j] && arr[i] !== 0 && arr[j] !== 0) inv++;
    }
  }
  return inv;
}
...
export function calculateWinner(arr) {
  ...
  for (let i = 0; i < 3; i++)
    for (let j = 0; j < 3; j++) if (arr[i][j] !== a[i][j]) return false;
  return true;
}
